...
const WAD_INSTALL_PATH = getWADInstallPath();

const WAD_EXE_MD5 = '50d694ebfaa622ef7e4061c1bf52efe6';
const WAD_GUID = 'DDCD58BF-37CF-4758-A15E-A60E7CF20E41';

async function getWADInstallPath () {
  const paths = [
    process.env['ProgramFiles(x86)'],
    process.env.ProgramFiles,
    'C:\\\\Program Files',
  ].map((folder) => path.resolve(folder, 'Windows Application Driver', 'WinAppDriver.exe'));
  return await paths.find(async (path) => await fs.exists(path));
}
